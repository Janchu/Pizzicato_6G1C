	...
	public void addTilaus(Tilaus tilaus, Kayttaja tilaaja) throws SQLException {
		...
		try {
			...
			if(rs.next() == false) { 
				...
				stmtInsert = connection.prepareStatement("INSERT INTO asiakkaantilaus (kayttaja_id, tilaus_id) VALUES(?, 'last_insert_id()')");
				stmtInsert.setInt(1, tilaaja.getId());
				stmtInsert.executeUpdate();
				stmtInsert.close();
				...
				System.out.println(tilaus.getTila());
				stmtInsert.setString(2, tilaus.getMaksutapa());
				System.out.println(tilaus.getMaksutapa());
				stmtInsert.setString(3, tilaus.getToimitus());
				System.out.println(tilaus.getToimitus());
